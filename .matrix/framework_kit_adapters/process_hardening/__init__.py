...
ASSET_ID = "as_codex_process_hardening"
COMPONENT_ID = "process_hardening"
ACCEPTED_VERSION = "v0001_initial"
EXPECTED_PLAN_HASH = "4d6a36ab7187565d071f8c444042190875d05fa5cbd2447e6c60633bc07d61d2"
...
SOURCE_FILES = ('BUILD.bazel', 'Cargo.toml', 'README.md', 'src/lib.rs')
...
def gate_probe() -> bool:
    ...
    return (ENGINE_ROOT.is_dir() and manifest().get("status") == "accepted" and manifest().get("expected_plan_hash") == EXPECTED_PLAN_HASH and len(rows) == len(SOURCE_FILES)
            and all((p := ENGINE_ROOT / r["target_relpath"]).is_file() and hashlib.sha256(p.read_bytes()).hexdigest() == (r.get("sha256_full") or r["sha256"]) for r in rows))
