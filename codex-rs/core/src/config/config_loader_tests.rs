...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(codex_paths::CODEX_HOME_DIR)).await?;
    tokio::fs::create_dir_all(project_root.join(codex_paths::CODEX_HOME_DIR)).await?;
    tokio::fs::write(project_root.join(codex_paths::GIT_DIR), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(codex_paths::CODEX_HOME_DIR).join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(codex_paths::CODEX_HOME_DIR).join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    assert_eq!(project_layers[0].as_path(), nested.join(codex_paths::CODEX_HOME_DIR).as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(codex_paths::CODEX_HOME_DIR).as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(codex_paths::CODEX_HOME_DIR)).await?;
    tokio::fs::create_dir_all(worktree_child.join(codex_paths::CODEX_HOME_DIR)).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(codex_paths::CODEX_HOME_DIR),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(codex_paths::CODEX_HOME_DIR),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(codex_paths::CODEX_HOME_DIR),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(codex_paths::CODEX_HOME_DIR),
        Some("worktree-child"),
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(repo_root.join(codex_paths::GIT_DIR)).await?;
    ...
    .await?;
    write_project_hook_config(
        &project_root.join(codex_paths::CODEX_HOME_DIR),
        /*foo*/ None,
        "echo project root hook",
    )
    .await?;
    write_project_hook_config(
        &nested.join(codex_paths::CODEX_HOME_DIR),
        /*foo*/ None,
        "echo nested hook",
    )
    .await?;
    ...
    assert_eq!(
        project_layers[0].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(nested.join(codex_paths::CODEX_HOME_DIR))?)
    );
    assert_eq!(
        project_layers[1].hooks_config_folder(),
        Some(AbsolutePathBuf::from_absolute_path(
...
"#;
    tokio::fs::write(project_root.join(codex_paths::CODEX_HOME_DIR).join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(codex_paths::CODEX_HOME_DIR).join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(codex_paths::CODEX_HOME_DIR).join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(codex_paths::CODEX_HOME_DIR).join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn project_layer_is_added_when_dot_codex_exists_without_config_toml() -> std::io::Result<()> {
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_codex_folder: AbsolutePathBuf::from_absolute_path(project_root.join(codex_paths::CODEX_HOME_DIR))?,
        },
        TomlValue::Table(toml::map::Map::new()),
    );
...
async fn codex_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    ...
    let codex_home = home_dir.join(codex_paths::CODEX_HOME_DIR);
    tokio::fs::create_dir_all(&codex_home).await?;
    tokio::fs::write(
        codex_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
async fn codex_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    ...
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(codex_paths::CODEX_HOME_DIR);
    let nested_dot_codex = nested.join(codex_paths::CODEX_HOME_DIR);

    tokio::fs::create_dir_all(&nested_dot_codex).await?;
    tokio::fs::create_dir_all(project_root.join(codex_paths::GIT_DIR)).await?;
    tokio::fs::write(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(codex_paths::CODEX_HOME_DIR)).await?;
    tokio::fs::write(project_root.join(codex_paths::GIT_DIR), "gitdir: here").await?;
    tokio::fs::write(nested.join(codex_paths::CODEX_HOME_DIR).join(CONFIG_TOML_FILE), "foo =").await?;
